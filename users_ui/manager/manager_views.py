...
def download_rekomendasi_template(request):
    """
    Download Excel template for Global Rekomendasi (Weekly Plan).
    Contains sheets for common parameters with pre-filled Day rows.
    """
    ...
    try:
        output = io.BytesIO()
        # Default engine (likely openpyxl)
        with pd.ExcelWriter(output) as writer:
            # Standard columns
            columns = ['Hari', 'Sarapan', 'Snack', 'Makan Siang', 'Makan Malam', 'Olahraga', 'Link']
            # Standard days
            days = ['Senin', 'Selasa', 'Rabu', 'Kamis', 'Jumat', 'Sabtu', 'Minggu']
            
            # Create a basic dataframe for the template
            ...
            # Common parameters as sheets
            params = ['Gula Darah Puasa', 'Gula Darah Sewaktu', 'Tekanan Darah', 'Cholesterol', 'Asam Urat', 'Global']
            
            for param in params:
                sheet_name = param[:31] # Excel sheet name limit
        ...
        response["Content-Disposition"] = 'attachment; filename="Template_Rekomendasi_Weekly.xlsx"'
        return response

    except Exception as e:
        request.session["error_message"] = f"Failed to generate template: {e}"
        # Fallback to referrer or dashboard
        return redirect(request.META.get('HTTP_REFERER', reverse("manager:dashboard")))
...
def upload_rekomendasi_xls(request):
    if not request.session.get("authenticated") or request.session.get("user_role") not in ["Manager", "nurse"]:
        return redirect("accounts:login")
...
def well_unwell_summary_json(request):
    ...
    if not request.session.get("authenticated") or role not in ["Manager", "Tenaga Kesehatan", "nurse"]:
...
def rekomendasi_global_create_json(request):
    ...
    """
    role = request.session.get("user_role")
    if not request.session.get("authenticated") or role not in ["Manager", "nurse"]:
        return JsonResponse({"error": "unauthorized"}, status=401)
    import json
    try:
        payload = json.loads(request.body.decode("utf-8"))
...
def rekomendasi_global_delete_json(request, rec_id):
    ...
    role = request.session.get("user_role")
    if not request.session.get("authenticated") or role not in ["Manager", "nurse", "Tenaga Kesehatan"]:
        return JsonResponse({"error": "unauthorized"}, status=401)
    try:
        rec = RekomendasiKesehatan.objects.get(id=int(rec_id))
        rec.delete()
